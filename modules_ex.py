...
def get_shuffled_cards():
        cards_features = [ "diamonds ♦","hearts ♥","spades ♠","clubs ♣"]
        cards_letters_features = ['A','V','D','K'] #not sure
        list_letters = list(itertools.product(cards_letters_features,cards_features))
        list_numbers = list(itertools.product(range(2,11),cards_features))
        list_numbers.extend(list_letters)
